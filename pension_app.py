...
# --- 4. יצירת דוח PDF (תיקון "בריחת" הערות ואיחוד טקסטים) ---
def generate_pdf_report(data_dict):
    ...
    # --- הערה משפטית משולבת בתחתית ---
    pdf.ln(15) # רווח מהטבלה
    pdf.set_font("ArialHeb", size=9)
    
    # כאן הטקסט שביקשת לגבי הקצבה + הדיסקליימר הכללי
    full_disclaimer = (
        ...
        "במידה ותהיה הכנסה נוספת מכל מקור שהוא (כגון שכר עבודה, עסק, שכירות וכיו''ב), חבות המס השנתית תגדל בהתאם למדרגות המס הקבועות בחוק. "
        "הנתונים הסופיים ייקבעו אך ורק על ידי רשויות המס בכפוף להגשת דוחות כחוק."
    )
    
    # שימוש ב-multi_cell עם רווח מוגדר למניעת השגיאה
    pdf.multi_cell(0, 6, txt=hb(full_disclaimer), align='R')

    return bytes(pdf.output())

# --- 5. ממשק המשתמש (ללא שינוי) ---
def main():
