...
def check_password():
    ...
    password = st.text_input("הזן סיסמה", type="password")
    
    if st.button("כניסה"):
        if password == "1234": # <--- כאן שנה את הסיסמה למה שתרצה!
...
def generate_pdf_report(data_dict):
    ...
    pdf.ln(5)
    if os.path.exists(font_reg): pdf.set_font("ArialHeb", size=14)
...
def main():
    ...
    if not check_password():
        st.stop() # עוצר את האפליקציה כאן אם הסיסמה לא הוזנה
    ...
    final_max_years = base_max_years - 1 if (start_year_choice > curr_yr and not is_late_retirement) else base_max_years
    ...
    total_tax_spread, report_data = 0, []
    ...
    if strategy == "פריסה אחורה": report_data = sorted(report_data, key=lambda x: x['שנה'])
    ...
    if st.button("📄 הפק דוח PDF"):
        pdf_data = {'agent_name': agent_name, 'client_name': client_name, 'client_id': client_id, 'total_grant': total_grant, 'exempt': exempt_val, 'taxable': taxable_val, 'tax_no_spread': tax_no_spread, 'tax_with_spread': total_tax_spread, 'savings': savings, 'years': num_years, 'table': report_data}
        try:
            pdf_bytes = generate_pdf_report(pdf_data)
            st.download_button("📥 הורד דוח PDF", data=pdf_bytes, file_name=f"Tax_Plan_{client_name}.pdf")
